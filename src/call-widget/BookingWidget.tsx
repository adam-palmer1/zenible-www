...
const BookingWidget: React.FC<BookingWidgetProps> = ({ config }) => {
  ...
  const handleSubmit = async (formData: BookingFormData): Promise<void> => {
    try {
      setSubmitting(true);
      setSubmitError(null);
      ...
      if (err.status === 409) {
        ...
        try {
          ...
          const dayData = data.days?.find((d: any) => d.date === selectedDate);
  ...
  const { availableDates, visitorAvailability } = useMemo(() => {
    ...
    const visitorSlots: Record<string, any[]> = {}; // { visitorDate: [{ hostDate, time, visitorTime }] }

    Object.entries(availabilityData).forEach(([hostDate, slots]) => {
      slots.forEach((time: any) => {
        try {
          ...
          const sourceHour = parseInt(sourceParts.find((p: any) => p.type === 'hour')!.value);
          const sourceMinute = parseInt(sourceParts.find((p: any) => p.type === 'minute')!.value);
