...
let cachedEnums: unknown | null = null;
...
export function useServiceEnums() {
  const [serviceStatuses, setServiceStatuses] = useState<unknown[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const hasFetched = useRef<boolean>(false);

  const fetchEnums = useCallback(async (force: boolean = false): Promise<unknown> => {
    // Return cached data if still valid
    const now = Date.now();
    if (!force && cachedEnums && (now - cacheTimestamp) < CACHE_TTL) {
      setServiceStatuses((cachedEnums as any).service_statuses || []);
      return cachedEnums;
    }
    ...
    try {
      ...
      const data = await servicesAPI.getEnums() as any;
  ...
  const getStatusByValue = useCallback((value: string): unknown | undefined => {
    return serviceStatuses.find((s: any) => s.value === value);
  }, [serviceStatuses]);

  // Helper to get status label
  const getStatusLabel = useCallback((value: string): string => {
    const status = getStatusByValue(value) as any;
