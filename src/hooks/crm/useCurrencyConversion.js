import { useState, useCallback, useEffect } from 'react';
import currencyConversionAPI from '../../services/api/crm/currencyConversion';

/**
 * Hook for currency conversion with caching
 * Provides methods to convert currencies and get exchange rates
 */
export const useCurrencyConversion = () => {
  const [rates, setRates] = useState({});
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [cacheTimestamp, setCacheTimestamp] = useState(null);

  // Cache TTL: 1 hour (matching backend)
  const CACHE_TTL = 60 * 60 * 1000;

  /**
   * Convert amount from one currency to another
   */
  const convert = useCallback(async (amount, fromCurrency, toCurrency) => {
    if (fromCurrency === toCurrency) {
      return amount;
    }

    setLoading(true);
    setError(null);

    try {
      const response = await currencyConversionAPI.convert(
        amount,
        fromCurrency,
        toCurrency
      );
      return response.converted_amount;
    } catch (err) {
      setError(err.message);
      console.error('Currency conversion failed:', err);
      return amount; // Fallback to original amount
    } finally {
      setLoading(false);
    }
  }, []);

  /**
   * Get exchange rates for multiple currencies
   */
  const getRates = useCallback(async (baseCurrency, currencies) => {
    // Check cache validity
    const now = Date.now();
    if (
      rates[baseCurrency] &&
      cacheTimestamp &&
      now - cacheTimestamp < CACHE_TTL
    ) {
      return rates[baseCurrency];
    }

    setLoading(true);
    setError(null);

    try {
      const response = await currencyConversionAPI.getRates(
        baseCurrency,
        currencies
      );

      // Update cache
      setRates((prev) => ({
        ...prev,
        [baseCurrency]: response.rates,
      }));
      setCacheTimestamp(now);

      return response.rates;
    } catch (err) {
      setError(err.message);
      console.error('Failed to get exchange rates:', err);
      return {};
    } finally {
      setLoading(false);
    }
  }, [rates, cacheTimestamp, CACHE_TTL]);

  /**
   * Batch convert multiple amounts
   */
  const batchConvert = useCallback(async (conversions) => {
    setLoading(true);
    setError(null);

    try {
      const response = await currencyConversionAPI.batchConvert(conversions);
      return response.results;
    } catch (err) {
      setError(err.message);
      console.error('Batch conversion failed:', err);
      return [];
    } finally {
      setLoading(false);
    }
  }, []);

  /**
   * Clear cache manually
   */
  const clearCache = useCallback(() => {
    setRates({});
    setCacheTimestamp(null);
  }, []);

  return {
    convert,
    getRates,
    batchConvert,
    clearCache,
    rates,
    loading,
    error,
  };
};
