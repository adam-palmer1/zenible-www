...
const BookingCancellation = () => {
  ...
  const formatDate = (dateStr, timezone = selectedTimezone) => {
    if (!dateStr) return '';
  ...
  const formatTime = (timeStr, timezone = selectedTimezone) => {
    if (!timeStr) return '';
  ...
  const handleTimeSelect = async (time) => {
    ...
    try {
      ...
      const result = await publicBookingAPI.rescheduleBooking(token, newStartDatetime, selectedTimezone);
