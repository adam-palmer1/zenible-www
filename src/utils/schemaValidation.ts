...
export function validateJSONSchema(schema: unknown): ValidationResult {
  ...
  const unknownFields = Object.keys(s).filter(key => !knownFields.includes(key));
...
export function formatJSONSchema(schema: unknown): string {
  try {
...
export function parseJSONSchema(schemaString: string): { schema: JSONSchema | null; error: string | null } {
  try {
...
export function getSchemaTemplates(): Record<string, SchemaTemplate> {
  return {
    simple: {
      name: 'Simple Input',
      schema: {
...
export function getSchemaFieldInfo(schema: JSONSchema | null | undefined): SchemaFieldInfo {
