...
const contactFilesAPI = {
   ...
   */
  list: (contactId: string, params: Record<string, unknown> = {}) => {
   ...
   */
  get: (contactId: string, fileId: string) => request(`/crm/contacts/${contactId}/files/${fileId}`, { method: 'GET' }),
   ...
   */
  upload: (contactId: string, fileData: unknown) => request(`/crm/contacts/${contactId}/files`, {
    method: 'POST',
   ...
   */
  update: (contactId: string, fileId: string, updateData: unknown) => request(`/crm/contacts/${contactId}/files/${fileId}`, {
    method: 'PATCH',
   ...
   */
  delete: (contactId: string, fileId: string) => request(`/crm/contacts/${contactId}/files/${fileId}`, { method: 'DELETE' }),
   ...
   */
  fileToUploadData: (file: File) => new Promise((resolve, reject) => {
