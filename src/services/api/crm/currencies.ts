...
const currenciesAPI = {
  ...
  addCurrencyToCompany: (currencyId: string) => request('/crm/currencies/company/enabled', {
    method: 'POST',
    body: JSON.stringify({ currency_id: currencyId }),
  }),

  /** Remove currency from company */
  removeCurrencyFromCompany: (associationId: string) => request(`/crm/currencies/company/enabled/${associationId}`, {
    method: 'DELETE',
  }),

  /** Set currency as default for company */
  setDefaultCurrency: (associationId: string) => request(`/crm/currencies/company/enabled/${associationId}/default`, {
    method: 'PATCH',
  }),

  /** Get company attribute by name */
  getCompanyAttribute: (attributeName: string) => request(`/crm/companies/current/attributes/${attributeName}`, {
    method: 'GET',
  }),
