...
export default function FirstSignInModal({ isOpen, onClose }: FirstSignInModalProps) {
  ...
  const fetchAvailablePlans = async () => {
    try {
      ...
      const response = await planAPI.getPublicPlans() as any;
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      ...
      setPlanError((err as any).message || 'Failed to select plan. Please try again.');
    } finally {
  ...
  const loadExistingData = async () => {
    ...
    setError(null);
    try {
      const companyData = await companiesAPI.getCurrent().catch(() => null) as any;
