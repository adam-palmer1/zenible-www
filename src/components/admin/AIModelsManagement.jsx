...
export default function AIModelsManagement() {
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      if (featureFilter) {
        ...
        response = await adminAPI.request(url, {
          method: 'GET'
        });
      } else {
  ...
  const handleUpdatePricing = async (modelId, pricingInput, pricingOutput) => {
    try {
      await adminAPI.updateOpenAIModel(modelId, {
      ...
      throw new Error(`Failed to update pricing: ${err.message}`);
    }
  };
