...
const ChevronUpIcon = ({ size = 14, className = '' }) => (
  <svg className={className} width={size} height={size} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function CharacterPlatformConfig({ characterId, characterName, isOpen, onClose, darkMode }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const configResponse = await adminAPI.getCharacterPlatforms(characterId);
  ...
  const handleUpdateConfig = async (platformId, data) => {
    try {
      setSaving(true);
  ...
  const handleDeleteConfig = async (platformId) => {
  ...
  const handlePriorityChange = async (config, direction) => {
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          {config.platform?.icon_svg && (
                            <div
                              className="w-8 h-8"
                              dangerouslySetInnerHTML={{ __html: config.platform.icon_svg }}
                            />
                          )}
                          <div>
