...
const validateQuestion = (question: any, questionIndex: number) => {
  const errors: any[] = [];
...
const validateQuiz = (quiz: any, quizIndex: number) => {
  const errors: any[] = [];
...
const validateBulkUpload = (data: any) => {
  const errors: any[] = [];
...
export default function BulkQuizUpload({ darkMode, onSuccess }: BulkQuizUploadProps) {
  const [inputMode, setInputMode] = useState<string>('file'); // 'file' or 'paste'
