...
export default function QuizzesManagement() {
  ...
  const [quizTags, setQuizTags] = useState<any[]>([]);
  ...
  const [expandedQuizId, setExpandedQuizId] = useState<any>(null);
  ...
  // Bulk delete states
  const [selectedQuizIds, setSelectedQuizIds] = useState<any[]>([]);
  ...
  const fetchQuizzes = async () => {
    ...
    try {
      ...
      const response = await (quizAPI as any).getQuizzes(params);
  ...
  const fetchQuizTags = async () => {
    try {
      const response = await (quizAPI as any).getQuizTags({ is_active: true });
      // Handle both direct array response and paginated object response
  ...
  const handleEditQuiz = async (quiz: any) => {
    try {
      ...
      const fullQuiz = await (quizAPI as any).getQuiz(quiz.id);
  ...
  const handleCloneQuiz = async (quiz: any) => {
    try {
      const fullQuiz = await (quizAPI as any).getQuiz(quiz.id);
  ...
  const handleSaveQuiz = async () => {
    try {
      ...
      for (let i = 0; i < quizForm.questions.length; i++) {
        ...
        const hasCorrectAnswer = question.answers.some((a: any) => a.is_correct);
        if (!hasCorrectAnswer) {
        ...
        const emptyAnswers = question.answers.filter((a: any) => !a.answer_text || a.answer_text.trim() === '');
      ...
      let savedQuiz;
      if (editingQuiz) {
        savedQuiz = await (quizAPI as any).updateQuiz(editingQuiz.id, data);

        // Handle questions and answers for existing quiz
        await syncQuestionsAndAnswers(editingQuiz.id);
      } else {
        savedQuiz = await (quizAPI as any).createQuiz(data);
  ...
  const syncQuestionsAndAnswers = async (quizId: any) => {
    try {
      // Build maps for comparison
      const originalQuestions = editingQuiz?.questions || [];
      const originalQMap = new Map(originalQuestions.map((q: any) => [q.id, q]));
      const currentQIds = new Set(
        quizForm.questions.filter((q: any) => q.id).map((q: any) => q.id)
      );
      ...
      for (const originalQ of originalQuestions) {
        if (!currentQIds.has(originalQ.id)) {
          await (quizAPI as any).deleteQuestion(originalQ.id);
        }
      }
      ...
      for (let i = 0; i < quizForm.questions.length; i++) {
        ...
        const answers = (question.answers || []).map((answer: any, j: number) => ({
        ...
        if (question.id && originalQMap.has(question.id)) {
          ...
          await (quizAPI as any).updateQuestion(question.id, questionData);
        } else {
          // CREATE new question (no ID or new question)
          await (quizAPI as any).addQuestion(quizId, questionData);
        }
      }
    } catch (err: any) {
  ...
  const handleDeleteQuiz = async () => {
    await deleteConfirmation.confirmDelete(async (quiz) => {
      try {
        await (quizAPI as any).deleteQuiz(quiz.id);
        fetchQuizzes();
      } catch (err: any) {
  ...
  const handleToggleActive = async (quiz: any) => {
    ...
    setQuizzes(prevQuizzes =>
      prevQuizzes.map((q: any) =>
        q.id === quiz.id ? { ...q, is_active: !q.is_active } : q
      )
    );

    try {
      await (quizAPI as any).updateQuiz(quiz.id, { is_active: !quiz.is_active });
    } catch (err: any) {
      // Revert on error
      setQuizzes(prevQuizzes =>
        prevQuizzes.map((q: any) =>
  ...
  const handleBulkUploadSuccess = async () => {
    ...
    if (editingQuiz) {
      const fullQuiz = await (quizAPI as any).getQuiz(editingQuiz.id);
      setQuizForm({
        ...quizForm,
        questions: fullQuiz.questions || []
      });
  ...
  const handleDeleteQuestion = (index: number) => {
    const updatedQuestions = quizForm.questions.filter((_: any, i: number) => i !== index);
    setQuizForm({ ...quizForm, questions: updatedQuestions });
  };
  ...
  const getTagNames = (tagIds: any[]) => {
    if (!tagIds || tagIds.length === 0) return 'No tags';
    const names = tagIds.map((id: any) => {
      const tag = quizTags.find((t: any) => t.id === id);
      return tag ? tag.name : 'Unknown';
    });
    return names.join(', ');
  };

  const getTags = (tagIds: any[]) => {
    if (!tagIds || tagIds.length === 0) return [];
    return tagIds.map((id: any) => quizTags.find((t: any) => t.id === id)).filter(Boolean);
  };
  ...
  const calculateTotalPoints = (questions: any[]) => {
    return questions.reduce((sum: number, q: any) => sum + (parseInt(q.points) || 0), 0);
  };

  const determineQuestionType = (answers: any[]) => {
  ...
  const handleSelectAllQuizzes = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedQuizIds(quizzes.map((quiz: any) => quiz.id));
    } else {
      setSelectedQuizIds([]);
    }
  };

  const handleSelectQuiz = (quizId: any) => {
    if (selectedQuizIds.includes(quizId)) {
      setSelectedQuizIds(selectedQuizIds.filter((id: any) => id !== quizId));
    } else {
  ...
  const handleBulkDeleteQuizzes = async () => {
    ...
    try {
      const result = await (quizAPI as any).bulkDeleteQuizzes({
  ...
  const getSelectedQuizzes = () => {
    return quizzes.filter((quiz: any) => selectedQuizIds.includes(quiz.id));
  };
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className={`p-4 rounded-xl border ${darkMode ? 'bg-zenible-dark-card border-zenible-dark-border' : 'bg-white border-neutral-200'}`}>
            ...
            <div className={`text-2xl font-semibold mt-1 ${darkMode ? 'text-zenible-dark-text' : 'text-zinc-950'}`}>
              {quizzes.reduce((sum: number, q: any) => sum + (q.question_count || 0), 0)}
            </div>
          </div>
        </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => deleteConfirmation.requestDelete(quiz)}
          ...
          <div className={`w-full max-w-4xl mx-4 my-8 rounded-xl ${darkMode ? 'bg-zenible-dark-card' : 'bg-white'}`}>
            ...
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                      ...
                      {quizTags.map((tag: any) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              if (isSelected) {
                                setQuizForm({ ...quizForm, tag_ids: quizForm.tag_ids.filter((id: any) => id !== tag.id) });
                              } else {
              ...
              <div className="space-y-4">
                ...
                {quizForm.questions.map((question: any, qIndex: number) => (
            ...
            <div className={`px-6 py-4 border-t flex gap-2 ${darkMode ? 'border-zenible-dark-border' : 'border-neutral-200'}`}>
              <button
                ...
                disabled={
                  ...
                  quizForm.questions.some((q: any) =>
                    !q.answers ||
                    q.answers.length < 2 ||
                    !q.answers.some((a: any) => a.is_correct)
                  )
                }
          ...
          <div className={`w-full max-w-md mx-4 rounded-xl ${darkMode ? 'bg-zenible-dark-card' : 'bg-white'}`}>
            ...
            <div className="p-6">
              <p className={`${darkMode ? 'text-zenible-dark-text' : 'text-gray-700'}`}>
                Are you sure you want to delete the quiz &quot;{deleteConfirmation.item.title}&quot;?
              </p>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className={`max-h-60 overflow-y-auto rounded-lg border ${darkMode ? 'border-zenible-dark-border' : 'border-gray-200'}`}>
                      <ul className={`divide-y ${darkMode ? 'divide-zenible-dark-border' : 'divide-gray-200'}`}>
                        {getSelectedQuizzes().map((quiz: any) => (
                ...
                <div className="space-y-4">
                  ...
                  {bulkDeleteResult.failed > 0 && bulkDeleteResult.errors && bulkDeleteResult.errors.length > 0 && (
                    <div className="space-y-2">
                      ...
                      <div className={`max-h-60 overflow-y-auto rounded-lg border ${darkMode ? 'border-zenible-dark-border' : 'border-gray-200'}`}>
                        <ul className={`divide-y ${darkMode ? 'divide-zenible-dark-border' : 'divide-gray-200'}`}>
                          {bulkDeleteResult.errors.map((error: any, index: number) => (
