...
const CurrencyExchangeWidget = ({ settings = {}, widgetId = 'currency-exchange' }: CurrencyExchangeWidgetProps) => {
  ...
  const getTrend = () => {
    ...
    const getRate = (d: any) => parseFloat(d.rate || d.exchange_rate || d.value || 0);
  ...
  const renderChart = () => {
    ...
    const points: ChartPoint[] = rates.map((rateVal, i) => {
      const x = paddingLeft + (i / (rates.length - 1)) * chartWidth;
      const y = paddingTop + chartHeight - ((rateVal - min) / range) * chartHeight;
      return { x, y, rate: rateVal, date: historicalData[i]?.date };
    });
