...
const WidgetGrid = ({ onOpenCustomizer }: WidgetGridProps) => {
  ...
  const moveWidget = useCallback((dragIndex: number, hoverIndex: number) => {
    ...
    const visibleIds = visibleWidgets.map((w: any) => w.id);
    ...
    const dragRealIndex = allWidgets.findIndex((w: any) => w.id === dragWidgetId);
    const hoverRealIndex = allWidgets.findIndex((w: any) => w.id === hoverWidgetId);
  ...
  // Handle hide widget
  const handleHideWidget = useCallback(async (widgetId: string) => {
    const updated = (widgetLayout.widgets || []).map((w: any) =>
 ...
 */
const SortableWidgetGrid = ({ onOpenCustomizer }: SortableWidgetGridProps) => {
  return (
