...
const RecentExpensesWidget = ({ settings = {}, isHovered = false }: RecentExpensesWidgetProps) => {
  ...
  const getExpenseTitle = (expense: any): string => {
  ...
  const getCategoryName = (expense: any): string => {
  ...
  const getCategoryColor = (expense: any): string => {
    return expense.expense_category?.color || '#6b7280';
  };

  const handleViewAll = () => navigate('/finance/expenses');
  const handleExpenseClick = (id: string) => navigate(`/finance/expenses?expense=${id}`);
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">
                        {getCategoryName(expense)} &bull; {formatDate(expense.expense_date)}
                      </p>
                    </div>
                  </div>
