...
export default function EventCard({ event, darkMode, onRegisterClick, onUnregisterClick }) {
  ...
  const getHostInitial = (name) => {
    return name ? name.charAt(0).toUpperCase() : '?';
  };

  // Fetch required plan details
  const fetchRequiredPlan = async (planId) => {
            ...
            <div className="relative shrink-0 rounded-full overflow-hidden" style={{ width: '40px', height: '40px' }}>
              {primaryHost.image_url ? (
                <img
                  ...
                  onError={(e) => {
                    // Replace the image with initials fallback
