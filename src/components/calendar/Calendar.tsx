...
const getAppointmentKey = (appointment: any) => {
...
const isRecurringAppointment = (appointments: any[], appointmentId: any) => {
...
export default function Calendar() {
  ...
  const [scopeDialogAppointment, setScopeDialogAppointment] = useState<any>(null);
  ...
  const DEFAULT_APPOINTMENT_COLORS = useMemo(() => {
    ...
    (appointmentTypes as any[]).forEach((type: any) => {
  ...
  const [visibleTypes, setVisibleTypes] = useState<string[]>(() => {
    const allTypeValues = (appointmentTypes as any[]).map((t: any) => t.value);
  ...
  useEffect(() => {
    if (!enumsLoading && (appointmentTypes as any[]).length > 0) {
      const allTypeValues = (appointmentTypes as any[]).map((t: any) => t.value);
  ...
  const getAppointmentColor = useCallback((appointment: any) => {
    ...
    if (googleTokenId && (googleAccounts as any[]).length > 0) {
      const account = (googleAccounts as any[]).find((acc: any) => String(acc.id) === String(googleTokenId));
      if (account?.color) {
    ...
    const typeObj = (appointmentTypes as any[]).find((t: any) => t.value === appointmentType);
  ...
  const isAppointmentReadOnly = useCallback((appointment: any) => {
  ...
  // Handle drag start
  const handleDragStart = (appointment: any, event: React.DragEvent) => {
  ...
  const handleDrop = async (day: Date, event: React.DragEvent, timeSlots: number[]) => {
    event.preventDefault();
  ...
  const getStatusLabel = (status: string) => {
    const statusObj = (appointmentStatuses as any[])?.find((s: any) => s.value === status);
              ...
              <div className="px-6 py-4 space-y-6">
                ...
                <div>
                  ...
                  {enumsLoading ? (
                    ...
                    <div className="space-y-3">
                      {(appointmentTypes as any[]).map((type: any) => (
...
// ========== Week View Component ==========
function WeekView({ currentDate, appointments, timeSlots, onAppointmentClick, onNewAppointment, onDragStart, onDragOver, onDrop, onDragEnd, draggingAppointment, dragPreview, getAppointmentColor, isRecurringAppointment, isAppointmentReadOnly, getStatusColor, getStatusLabel }: any) {
  ...
  const calculateAppointmentLayout = (dayAppointments: any[]) => {
...
function MonthView({ currentDate, appointments, onAppointmentClick, onNewAppointment, getAppointmentColor, isRecurringAppointment, isAppointmentReadOnly, getStatusColor, getStatusLabel }: any) {
...
// ========== Day View Component ==========
function DayView({ currentDate, appointments, timeSlots, onAppointmentClick, onNewAppointment, onDragStart, onDragOver, onDrop, onDragEnd, draggingAppointment, dragPreview, getAppointmentColor, isRecurringAppointment, isAppointmentReadOnly, getStatusColor, getStatusLabel }: any) {
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const [appointmentsBelow, setAppointmentsBelow] = useState<any[]>([]);
  const [currentTime, setCurrentTime] = useState(new Date());
...
function MiniCalendar({ currentDate, onDateSelect, appointments }: any) {
...
function GoogleCalendarConnector({ isConnected, accounts, primaryAccount, onAddAccount, onSetPrimary, onDisconnect, onRename, onColorChange, onToggleReadOnly, onSync }: any) {
...
function UpcomingSchedule({ appointments, onAppointmentClick }: any) {
