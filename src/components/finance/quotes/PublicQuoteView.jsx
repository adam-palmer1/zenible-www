...
const PublicQuoteView = () => {
  ...
  const itemLevelTax = items.reduce((sum, item) => {
    const itemTax = item.taxes?.reduce((t, tax) => t + (tax.tax_amount || 0), 0) || parseFloat(item.tax_amount || 0);
    return sum + itemTax;
  }, 0);
                  ...
                  <tbody>
                    {items.length === 0 ? (
                      ...
                      items.map((item, index) => {
                        ...
                        const itemTaxAmount = item.taxes?.reduce((sum, t) => sum + (t.tax_amount || 0), 0) || parseFloat(item.tax_amount || 0);
