...
const TypeBreakdownChart = ({ data, loading, currencySymbol = '$' }) => {
  ...
  const chartData = {
    labels: data.map((item) => formatTypeLabel(item.transaction_type)),
    datasets: [
      {
        data: data.map((item) => parseFloat(item.total_amount) || 0),
        backgroundColor: data.map(
          (item) => TYPE_COLORS[item.transaction_type]?.bg || 'rgba(156, 163, 175, 0.8)'
        ),
        borderColor: data.map(
          (item) => TYPE_COLORS[item.transaction_type]?.border || '#9ca3af'
        ),
        borderWidth: 2,
      },
    ],
