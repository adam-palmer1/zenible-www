...
/**
 * ExpenseAllocationSummaryBar - Visual summary bar showing expenses allocated to an entity
 *
 * @param {Object} props
 * @param {string} props.entityType - Type of entity (payment, invoice, project, contact)
 * @param {string|number} props.entityId - ID of the entity
 * @param {number} props.totalAmount - Total entity amount for reference display
 ...
 * @param {number} props.refreshKey - Key to trigger refetch
 */
const ExpenseAllocationSummaryBar = ({
  entityType,
  entityId,
  totalAmount = 0,
  currency = 'GBP',
  ...
  useEffect(() => {
    const fetchAllocations = async () => {
      ...
      try {
        setLoading(true);
        const response = await expensesAPI.getExpensesByEntity(entityType, entityId);
        ...
        const expenseAllocations = expenses.map((expense) => {
          ...
          const allocation = expense.allocations?.find(
            (a) => a.entity_type === entityType && String(a.entity_id) === String(entityId)
          );
  ...
  // Calculate totals
  const { totalAllocatedAmount, totalPercentageOfEntity } = useMemo(() => {
    const allocatedSum = allocations.reduce((sum, alloc) => sum + (alloc.amount || 0), 0);
  ...
  if (compact) {
    ...
    const avgPercentage = allocations.length > 0
      ? allocations.reduce((sum, a) => sum + a.percentage, 0) / allocations.length
      : 0;
