 ...
 */
const DocumentLineItems: React.FC<DocumentLineItemsProps> = ({
  items,
  currencyCode = 'USD',
  numberFormat = null,
  onAddItem,
  ...
  useEffect(() => {
    if (tableRef.current) {
      const textareas = tableRef.current.querySelectorAll('.auto-grow-textarea') as NodeListOf<HTMLTextAreaElement>;
                ...
                <tr key={item.id || index}>
                  ...
                  <td className="px-4 py-3 align-top">
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateItem(index, 'price', e.target.value)}
                      min="0"
                      step="0.01"
                      placeholder="0"
                      autoComplete="off"
