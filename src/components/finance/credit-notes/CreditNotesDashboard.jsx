...
const CreditNotesDashboard = () => {
  ...
  const loadCreditNotes = async () => {
    try {
      ...
      const response = await creditNotesAPI.list(filters);
  ...
  const StatusBadge = ({ status }) => {
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium ${CREDIT_NOTE_STATUS_COLORS[status] || 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'}`}>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {filterStatus !== 'all' && (
                    <div className="flex items-center gap-1 px-3 py-1.5 bg-purple-100 text-purple-700 rounded-full text-sm">
                      <span>Status:</span>
                      <span className="font-medium">{CREDIT_NOTE_STATUS_LABELS[filterStatus] || filterStatus}</span>
                      <button
