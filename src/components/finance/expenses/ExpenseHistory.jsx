...
const ExpenseHistory = ({ expenseId }) => {
  ...
  // Format changes with reference data
  const formattedChanges = history.changes.map(change =>
    formatExpenseChange(change, { categories, vendors })
  );
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm design-text-secondary mb-1">
              {change.displayOldValue && (
                <span className="line-through mr-2">{change.displayOldValue}</span>
              )}
              {change.displayOldValue && change.displayNewValue && '→ '}
              {change.displayNewValue && (
                <span className="font-medium design-text-primary">{change.displayNewValue}</span>
              )}
              {!change.displayOldValue && !change.displayNewValue && (
