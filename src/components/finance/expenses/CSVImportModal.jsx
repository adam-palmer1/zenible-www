 ...
 * Multi-step wizard for CSV expense import
 */
  ...
  const handleFileSelect = async (selectedFile) => {
    ...
    try {
      ...
      data.forEach((row, index) => {
        ...
        if (!validation.valid) {
          errors.push({
            rowIndex: index + 2, // +2 because index is 0-based and we skip header
            row,
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < validRows.length; i++) {
      try {
        ...
        // Add selected currency if not in row
        ...
        <table className="min-w-full divide-y design-divide text-sm">
          ...
          <tbody className="divide-y design-divide">
            {parsedData.slice(0, 50).map((row, index) => {
              const error = validationErrors.find(e => e.rowIndex === index + 2);
              return (
    ...
    <div className="space-y-6">
      ...
      <div className="design-bg-secondary rounded-lg p-4">
        ...
        <ul className="text-sm design-text-secondary space-y-1">
          <li>• Total rows: {parsedData.length}</li>
          <li>• Valid rows: {parsedData.length - validationErrors.length}</li>
          <li>• Rows with errors: {validationErrors.length}</li>
        </ul>
        {validationErrors.length > 0 && (
