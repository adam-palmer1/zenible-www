...
const SortableHeader: React.FC<SortableHeaderProps> = ({ label, sortKey, currentSortBy, currentSortOrder, onSort }) => {
...
const ExpenseList: React.FC = () => {
  ...
  const [allocationExpense, setAllocationExpense] = useState<any>(null);
  ...
  useEffect(() => {
    if (urlExpenseId && expenses.length > 0 && !loading) {
      const expense = expenses.find((e: any) => e.id === urlExpenseId);
      if (expense) {
  ...
  const getExpenseVendorName = (expense: any) => {
  ...
  const handleSelectAllVendors = () => {
    const allVendorIds = vendors.map((v: any) => v.id);
  ...
  return (
    <div>
                    ...
                    <td className="px-4 py-3 text-sm" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                      <div className="w-28">
                        <ExpenseAllocationBar
                          totalPercentage={(expense.allocations || []).reduce((sum: number, a: any) => sum + parseFloat(a.percentage || 0), 0)}
      ...
      {openActionMenuId && (() => {
        const expense = filteredExpenses.find((e: any) => e.id === openActionMenuId);
