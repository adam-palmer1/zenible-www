...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ invoice: invoiceProp = null, onSuccess, isInModal = false }) => {
  ...
  const { id } = useParams();
  const { createInvoice, updateInvoice } = useInvoices() as any;
  ...
  // State
  const [invoice, setInvoice] = useState<any>(invoiceProp);
  ...
  // Totals
  const [documentTaxes, setDocumentTaxes] = useState<any[]>([]); // Array of { tax_name, tax_rate }
  const [discountType, setDiscountType] = useState('percentage');
  const [discountValue, setDiscountValue] = useState<any>(0);
  const [depositType, setDepositType] = useState<string | null>(null);
  const [depositValue, setDepositValue] = useState<any>(0);
  ...
  const [recurringType, setRecurringType] = useState((RECURRING_TYPE as any).MONTHLY);
  ...
  const [unbilledHoursData, setUnbilledHoursData] = useState<any>(null);
  ...
  const [pendingProjectAllocations, setPendingProjectAllocations] = useState<any[]>([]); // Array of { project_id, percentage }
  const [showMarkBilledModal, setShowMarkBilledModal] = useState(false);
  const [markingAsBilled, setMarkingAsBilled] = useState(false);
  const [pendingInvoiceResult, setPendingInvoiceResult] = useState<any>(null);
  ...
  useEffect(() => {
    const loadInvoice = async () => {
      if (id && !invoiceProp) {
        try {
          setLoading(true);
          const data = await (invoicesAPI as any).get(id);
          setInvoice(data);
        } catch (error) {
  ...
  useEffect(() => {
    if (invoice) {
      ...
      const normalizedItems = (invoice.invoice_items || invoice.items || []).map((item: any) => ({
        ...item,
      ...
      setRecurringType(invoice.recurring_type || (RECURRING_TYPE as any).MONTHLY);
  ...
  useEffect(() => {
    const loadNextNumber = async () => {
      if (!invoice) {
        try {
          const data = await (invoicesAPI as any).getNextNumber();
          if (data?.next_number) {
  ...
  const handleClientSelect = async (clientId: string) => {
    ...
    if (!invoice && clientId) {
      ...
      try {
        const fullContact = await (contactsAPI as any).get(clientId);
      ...
      if (!hasCheckedUnbilledHours) {
        ...
        try {
          ...
          const response = await (billableHoursAPI as any).getByContact(clientId, {
          ...
          if (response && response.items && response.items.length > 0) {
            ...
            if (projectIds.length > 0) {
              const projectPromises = projectIds.map(async (projectId: string) => {
                try {
                  const project = await (projectsAPI as any).get(projectId);
            ...
            const enrichedItems = response.items.map((item: any) => ({
              ...item,
  ...
  const handleAddUnbilledHours = (lineItems: any[]) => {
    // Extract billable hour IDs from line items for later marking as billed
    const allBillableHourIds = lineItems.flatMap((item: any) => item._billable_hour_ids || []);
    setPendingBillableHourIds(allBillableHourIds);

    // Calculate project allocations based on hours proportion
    const totalHours = lineItems.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0);
    const projectAllocations = lineItems
      .filter((item: any) => item._project_id != null)
      .map((item: any) => ({
    ...
    const totalPercentage = projectAllocations.reduce((sum: number, a: any) => sum + a.percentage, 0);
    if (projectAllocations.length > 0 && totalPercentage !== 100) {
      const diff = 100 - totalPercentage;
      const largestIdx = projectAllocations.reduce((maxIdx: number, curr: any, idx: number, arr: any[]) =>
    ...
    setItems((prevItems: any[]) => [...prevItems, ...cleanedItems]);
  ...
  const handleMarkHoursAsBilled = async () => {
    ...
    try {
      ...
      await (billableHoursAPI as any).bulkUpdateByContact(
        contactId,
  ...
  const handleSave = async (saveStatus = (INVOICE_STATUS as any).DRAFT, openSendDialog = false) => {
    // Validation
    if (!contactId) {
    ...
    try {
      ...
      const invoiceData = {
        ...
        custom_every: pricingType === 'recurring' && recurringType === (RECURRING_TYPE as any).CUSTOM ? customEvery : undefined,
        custom_period: pricingType === 'recurring' && recurringType === (RECURRING_TYPE as any).CUSTOM ? customPeriod : undefined,
      ...
      if (pendingProjectAllocations.length > 0 && result?.id) {
        try {
          await (invoicesAPI as any).updateAllocations(result.id, pendingProjectAllocations);
        } catch (allocError) {
  ...
  const formContent = (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      {isInModal && (
        <div className="flex items-center justify-between gap-3 pt-4 border-t border-gray-200">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => handleSave(invoice?.status && invoice.status !== 'draft' ? invoice.status : (INVOICE_STATUS as any).DRAFT)}
              disabled={saving}
            ...
            <button
              onClick={() => handleSave((INVOICE_STATUS as any).SENT, true)}
              disabled={saving}
      ...
      <TaxModal
        ...
        onSave={(taxes: any) => setDocumentTaxes(taxes)}
      ...
      <InvoiceSettingsModal
        ...
        contact={allContacts.find((c: any) => c.id === contactId)}
      ...
      <SendInvoiceDialog
        ...
        contact={savedInvoice && allContacts.find((c: any) => c.id === savedInvoice.contact_id)}
