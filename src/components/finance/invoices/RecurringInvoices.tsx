...
const RecurringInvoices: React.FC = () => {
  ...
  const [cancelConfirmModal, setCancelConfirmModal] = useState<{ isOpen: boolean; invoice: any }>({ isOpen: false, invoice: null });
  ...
  // Calculate stats
  const stats = useMemo(() => {
    const active = templates.filter((t: any) => t.recurring_status === 'active').length;
    const paused = templates.filter((t: any) => t.recurring_status === 'paused').length;
    const totalValue = templates.reduce((sum: number, t: any) => sum + parseFloat(t.total || 0), 0);
              ...
              <table className="min-w-full divide-y divide-[#e5e5e5] dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-[#e5e5e5] dark:divide-gray-700">
                  {loading ? (
                    ...
                    filteredTemplates.map((invoice: any) => (
                      <tr
                        key={invoice.id}
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm relative" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                          <div className="relative">
                            <button
