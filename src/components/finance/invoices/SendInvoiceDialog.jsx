...
/**
 * SendInvoiceDialog Component
 *
 * Dialog for sending invoices via email with template selection
 *
 * @param {boolean} isOpen - Dialog open state
 * @param {Function} onClose - Callback to close dialog
 * @param {Object} invoice - Invoice object to send
...
const SendInvoiceDialog = ({
  ...
  // Fetch effective template for invoice_send
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      showSuccess(`Invoice sent successfully to ${formData.to_email}`);
      onSuccess?.();
      onClose();
    } catch (error) {
              ...
              <>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows={8}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-zenible-primary focus:border-transparent dark:bg-gray-700 dark:text-white font-mono text-sm"
                    required
                  />
                </div>
