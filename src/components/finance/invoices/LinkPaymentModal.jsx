...
const LinkPaymentModal = ({ isOpen, onClose, invoice, onSuccess }) => {
  ...
  const loadPayments = async () => {
    try {
      ...
      const currencyFilteredPayments = rawPayments.filter(payment => {
      ...
      const paymentsWithUnallocated = await Promise.all(
        currencyFilteredPayments.map(async (payment) => {
          try {
            const unallocatedData = await paymentsAPI.getUnallocated(payment.id);
            return {
              ...payment,
      ...
      const availablePayments = paymentsWithUnallocated.filter(
        payment => parseFloat(payment.unallocated_amount || 0) > 0
      );
