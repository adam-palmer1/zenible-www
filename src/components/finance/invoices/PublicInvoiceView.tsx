...
// Stripe logo SVG
const StripeLogo: React.FC<StripeLogoProps> = ({ className = 'h-5 w-5' }) => (
...
// PayPal logo SVG
const PayPalLogo: React.FC<PayPalLogoProps> = ({ className = 'h-5 w-5' }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
...
const StripeCardSetupForm: React.FC<StripeCardSetupFormProps> = ({ clientSecret, setupIntentId, shareCode, onSuccess, onError, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((setupIntent as any).status === 'succeeded') {
        // Optionally verify with backend
        try {
          await invoicesAPIAny.confirmCardSetup(shareCode, setupIntentId);
        } catch (confirmErr) {
...
interface StripeCardSetupSectionProps {
  shareCode: string;
  onSuccess: () => void;
  onError?: (err: any) => void;
  onBack: () => void;
  ...
  useEffect(() => {
    ...
    const initStripeSetup = async () => {
      try {
        ...
        const setupData = await invoicesAPIAny.createStripeSetupIntent(shareCode);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((paymentIntent as any).status === 'succeeded') {
        ...
        const statusResponse = await invoicesAPIAny.getStripePaymentStatus(shareCode, paymentId);
  ...
  useEffect(() => {
    ...
    const initStripe = async () => {
      try {
        ...
        const paymentData = await invoicesAPIAny.createStripePayment(shareCode, {
          amount,
        });
...
const PayPalPaymentSection: React.FC<PayPalPaymentSectionProps> = ({ shareCode, amount, currency, onBack }) => {
  ...
  const handlePayPalClick = async () => {
    ...
    try {
      ...
      const orderData = await invoicesAPIAny.createPayPalOrder(shareCode, {
        amount,
...
interface SavedCardPaymentInlineProps {
  shareCode: string;
  amountDue: number;
  currency: string;
  savedCard: any;
...
const SavedCardPaymentInline: React.FC<SavedCardPaymentInlineProps> = ({ shareCode, amountDue, currency, savedCard, onSuccess, onBack }) => {
  ...
  const handlePayWithSavedCard = async () => {
    ...
    try {
      const result = await invoicesAPIAny.payWithSavedCard(shareCode, {});
...
interface SavedCardPaymentSectionProps {
  shareCode: string;
  amountDue: number;
  currency: string;
  onSuccess: () => void;
...
const PublicInvoiceView: React.FC = () => {
  ...
  const loadInvoice = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await invoicesAPIAny.getPublicByShareCode(invoiceCode);
      setInvoice(data);
  ...
  const loadSavedCards = useCallback(async () => {
    ...
    try {
      ...
      const data = await invoicesAPIAny.getSavedCards(invoiceCode);
  ...
  const confirmDeleteCard = async () => {
    ...
    try {
      ...
      setDeleteCardError(null);
      await invoicesAPIAny.deleteSavedCard(invoiceCode, cardToDelete.id);
    ...
    const handlePayPalReturn = async () => {
      ...
      if (paypalStatus === 'approved') {
        ...
        if (orderId && storedShareCode) {
          ...
          try {
            const result = await invoicesAPIAny.capturePayPalOrder(storedShareCode, {
              order_id: orderId,
            });
