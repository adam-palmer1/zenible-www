...
const CurrenciesTab: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Enabled Currencies</h3>
          ...
          <div className="relative" ref={dropdownRef}>
            {showDropdown ? (
              <div className="relative">
                ...
                <div className="absolute top-full left-0 mt-1 w-64 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
                  {availableCurrencies.length === 0 ? (
                    ...
                    availableCurrencies.slice(0, 20).map((currency: any) => (
                      <button key={currency.id} onClick={() => handleAddCurrency(currency.id)} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex items-center justify-between">
                  ...
                  {availableCurrencies.length > 20 && (
                    <div className="px-4 py-2 text-xs text-gray-400 text-center border-t border-gray-200 dark:border-gray-700">Type to filter more currencies...</div>
                  )}
                </div>
              </div>
            ) : (
              <button onClick={() => setShowDropdown(true)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border-2 border-dashed border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-zenible-primary hover:text-zenible-primary transition-colors">
                <PlusIcon className="h-4 w-4" /><span>Add Currency</span>
              </button>
            )}
          </div>
        </div>
