          ...
          {services.map((service: any, index: number) => (
            <tr key={service.id} className={`border-b border-[#e5e5e5] dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 ${index === services.length - 1 ? 'border-b-0' : ''}`}>
              ...
              <td className="px-4 py-4 whitespace-nowrap">
                {service.status && (
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${(SERVICE_STATUS_COLORS as any)[service.status] || 'bg-gray-100 text-gray-800'}`}
                  >
