...
const AddProjectModal: React.FC<AddProjectModalProps> = ({ isOpen, onClose, project = null }) => {
  ...
  useEffect(() => {
    ...
    if (projectData) {
      ...
      if (projectData.service_assignments && Array.isArray(projectData.service_assignments)) {
        const serviceIds = projectData.service_assignments.map((sa: any) => sa.contact_service_id);
  ...
  const loadClientServices = async (contactId: string) => {
    if (!contactId) {
      setClientServices([]);
      return;
    }
  ...
  const handleServiceSelectionApply = (serviceIds: string[]) => {
    setSelectedServices(serviceIds);
  };
