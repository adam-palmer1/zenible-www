 ...
 */
const calculateContactValue = (contact: any) => {
...
interface SalesPipelineNewProps {
  contacts?: any[];
  statuses?: any[];
  globalStatuses?: any[];
  customStatuses?: any[];
  ...
  onUpdateContact?: (contactId: string, data: any, options?: any) => Promise<void>;
 ...
 */
const SalesPipelineNew: React.FC<SalesPipelineNewProps> = ({
  contacts = [],
  statuses = [],
  globalStatuses = [],
  customStatuses = [],
  ...
  const contactsByStatus = useMemo(() => {
    ...
    Object.keys(grouped).forEach((statusId) => {
      if (sortOrder === 'high_to_low' || sortOrder === 'low_to_high') {
        ...
        grouped[statusId].sort((a: any, b: any) => (a.sort_order || 0) - (b.sort_order || 0));
      }
    });
  ...
  const activeContact = useMemo(() => {
    ...
    return contacts.find((c: any) => c.id === activeId);
  ...
  const findColumnForContact = useCallback(
    (contactId: string) => {
      for (const [statusId, columnContacts] of Object.entries(contactsByStatus)) {
        if (columnContacts.some((c: any) => c.id === contactId)) {
          return statusId;
        }
      }
      return null;
    ...
    async (event: any) => {
      ...
      if (activeType === 'column') {
        ...
        if (activeColumnId !== overColumnId) {
          const oldIndex = statuses.findIndex((s: any) => s.id === activeColumnId);
          const newIndex = statuses.findIndex((s: any) => s.id === overColumnId);
      ...
      const isGlobalStatus = globalStatuses.some((s: any) => s.id === destinationColumnId);
  ...
  const columnIds = statuses.map((s: any) => `column-${s.id}`);
