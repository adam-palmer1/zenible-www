...
export const contactSchema = z.object({
  ...
  default_payment_terms: z.any().optional().transform(val => {
  ...
  currency_id: z.any().optional().transform(val => {
    if (!val || val === '' || val === 'null' || val === 'undefined') return null;
    return val;
  }),
  preferred_currency_id: z.any().optional().transform(val => {
    if (!val || val === '' || val === 'null' || val === 'undefined') return null;
    return val;
  }),
  invoice_payment_terms: z.any().optional().transform(val => {
    if (val === null || val === undefined || val === '' || val === 'null') return null;
    const num = typeof val === 'string' ? parseInt(val, 10) : val;
    if (isNaN(num)) return null;
    return Math.min(Math.max(num, 0), 365);
 ...
 * @param {Object} contact - Existing contact data (null for new contacts)
