...
interface BillableHourModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId: string;
  entry?: any;
...
const BillableHourModal: React.FC<BillableHourModalProps> = ({
  ...
  const effectiveDefaultCurrency = useMemo(() => {
    ...
    if (contactCurrencyId) {
      const foundById = companyCurrencies.find((c: any) => c.currency_id === contactCurrencyId);
      if (foundById) return foundById;
    }
    if (contactCurrencyCode) {
      const foundByCode = companyCurrencies.find((c: any) => c.currency?.code === contactCurrencyCode);
      if (foundByCode) return foundByCode;
    }
    // Second priority: project's default currency (by ID or code)
    if (projectCurrencyId) {
      const foundById = companyCurrencies.find((c: any) => c.currency_id === projectCurrencyId);
      if (foundById) return foundById;
    }
    if (projectCurrencyCode) {
      const foundByCode = companyCurrencies.find((c: any) => c.currency?.code === projectCurrencyCode);
      if (foundByCode) return foundByCode;
    }
  ...
  const selectedCurrencyObj = companyCurrencies.find((c: any) => c.currency_id === formData.currency_id);
  const currencyCode = selectedCurrencyObj?.currency?.code || effectiveDefaultCurrency?.currency?.code || 'USD';

  // Currency options for dropdown
  const currencyOptions = useMemo(() =>
    companyCurrencies.map((c: any) => ({
      value: c.currency_id,
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('start_time', e.target.value)}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('end_time', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-zenible-primary focus:border-transparent"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('is_billable', e.target.checked)}
            disabled={isInvoiced}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('hourly_rate', e.target.value)}
            ...
            <GenericDropdown
              ...
              onChange={(value: any) => handleInputChange('currency_id', value)}
