...
const ServiceDetailModal = ({
  ...
  const handleCreateAttribution = async (data) => {
  ...
  const handleCreateInvoiceLink = async (data) => {
    await createInvoiceLink(contactId, service.id, data);
    await loadInvoiceLinks();
    onServiceUpdate?.();
  };

  const handleDeleteInvoiceLink = async (invoiceLinkId) => {
      ...
      >
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {service.status && (
              <span className={`text-sm px-3 py-1 rounded-full ${SERVICE_STATUS_COLORS[service.status] || 'bg-gray-100 text-gray-800'}`}>
