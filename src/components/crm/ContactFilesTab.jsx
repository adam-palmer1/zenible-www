 ...
 */
const FileItem = ({ file, projects, onEdit, onDelete, onDownload }) => {
  ...
  const projectName = file.project_id
    ? projects.find(p => p.id === file.project_id)?.name || 'Unknown Project'
    : null;
 ...
 */
const DeleteConfirmModal = ({ file, isOpen, onConfirm, onCancel, isDeleting }) => {
 ...
 * @param {Array} projects - Contact's projects for assignment
 */
const ContactFilesTab = ({ contactId, projects = [] }) => {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const params = { include_download_urls: true };
      if (filterProjectId) {
        params.project_id = filterProjectId;
      }
      const response = await contactFilesAPI.list(contactId, params);
  ...
  // Handle file edit
  const handleEdit = async (fileId, updateData) => {
    try {
  ...
  const handleDownload = async (file) => {
    try {
      ...
      const fileData = await contactFilesAPI.get(contactId, file.id);
