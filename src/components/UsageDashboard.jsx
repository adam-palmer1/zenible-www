...
function UsageProgressBar({ current, limit, darkMode, showPercentage = true }) {
...
function FeatureBadge({ enabled, darkMode }) {
  return (
          ...
          {integrations && Object.keys(integrations).length > 0 && (
            <div>
              ...
              {expandedSections.integrations && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(integrations).filter(([, integration]) => integration != null).map(([key, integration]) => (
                    <div
                      key={key}
                      className={`p-4 rounded-lg ${darkMode ? 'bg-zenible-dark-bg' : 'bg-gray-50'}`}
                    >
